...
class TLDetector(object):
    ...
    def process_traffic_lights(self):
        ...
        for i, _light in enumerate(self.lights):
            # get the stopline index
            line = stop_line_positions[i]
            temp_wp_idx = self.get_closest_waypoint(line[0], line[1])
            # find closest stop line waypoint index
            _diff = temp_wp_idx - car_position
            if _diff >=0 and _diff < diff:
                diff = _diff
                light = _light
        ...
        if light:
            ...
            #rospy.logwarn("car: {0}; line {1} and state {2} and class {3}".format(car_position, line_wp_idx, light.state, state))
